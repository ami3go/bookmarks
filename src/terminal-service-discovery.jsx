        ...
        try {
            await modifyConfiguration(current => {
                ...
                if (additions.length === 0)
                    return current;
                return { ...current, services: [...current.services, ...additions] };
    ...
    return (
        <div className="bookmarks-discovery-floating">
            ...
            <Modal isOpen={open} onClose={() => !saving && setOpen(false)} variant="medium">
                                    ...
                                    return (
                                        <label className={`bookmarks-discovery-item${disabled ? ' is-disabled' : ''}`} key={candidate.port}>
                                            ...
                                            <div className="bookmarks-discovery-details">
                                                ...
                                                {terminalText && <span className="bookmarks-discovery-integration">{terminalText}</span>}
                                                {candidate.securityNotes?.map((note, index) => (
                                                    <span className="bookmarks-discovery-security-note" key={`${candidate.port}-${index}`}>{note}</span>
                                                ))}
                                            </div>
                ...
                <ModalFooter>
                    <Button variant="primary" onClick={addSelected} isDisabled={saving || loading || selectedCount === 0}>
